...
class Build : NukeBuild
{
    ...
    private const string prodViewsPath = "/DesktopModules/MyTestModule/resources/scripts/era-mymodule/";
    ...
    Target Compile => _ => _
        ...
        .Executes(() =>
        {
            ...
            // Build the .NET projects only. The front-end (module.web.esproj) is compiled
            // separately by the BuildFrontEnd target (npm run build) to avoid building it twice.
            DotNetBuild(s => s
                .SetProjectFile(Solution.GetProject("Module"))
                .SetConfiguration(Configuration)
    ...
    Target DeployFrontEnd => _ => _
        ...
        .Executes(() =>
        {
            ...
            (RootDirectory / "module.web" / "dist" / "era-mymodule").CopyToDirectory(scriptsDestination, ExistsPolicy.MergeAndOverwrite);
        });
